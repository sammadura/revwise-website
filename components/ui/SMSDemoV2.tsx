...
export default function SMSDemoV2({ compact = false }: SMSDemoV2Props) {
  ...
  // Auto-start on mount with longer delay
  useEffect(() => {
    const timer = setTimeout(() => {
      resetDemo();
    }, 2000);
  ...
  return (
    <div className={`relative mx-auto ${compact ? 'max-w-[20rem]' : 'max-w-sm'}`}>
      ...
      <div
        className={`absolute bg-gradient-to-r from-sage/40 via-petal/60 to-sage/40 rounded-[4rem] blur-2xl opacity-70 ${
          compact ? 'inset-0' : '-inset-4'
        }`}
      />
        ...
        <div className="bg-gray-100 rounded-[2.5rem] overflow-hidden">
          ...
          <div className={`${compact ? 'h-64' : 'h-80'} bg-gradient-to-b from-gray-50 to-gray-100 px-4 py-4 flex flex-col`}>
            <div className="space-y-3 flex-1">
              ...
              {/* Typing indicator */}
              {!visibleMessages.includes(4) && visibleMessages.length > 0 && visibleMessages.length < 4 && (
      ...
      {/* Decorative blurs — skip the upward ones in the hero so they don't cover proof copy */}
      {!compact && (
        <>
